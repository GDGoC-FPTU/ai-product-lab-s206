...
def evaluate_prompt(user_input: str) -> str:
    ...
    # Offline / Verification Fallback when API key is missing or SDK call fails
    input_lower = user_input.lower()
    if any(term in input_lower for term in ["2%", "1%", "3%", "4%", "cực kỳ gấp", "hết pin"]):
        return '{"action": "dispatch_mobile_charger", "reason": "Battery level under critical threshold of 5%. Cannot reach station safely."}'
    else:
        return '[DRAFT_ONLY] Cảm ơn quý khách đã sử dụng dịch vụ của Xanh SM! Chúc quý khách đi đường thượng lộ bình an.'
...
if __name__ == "__main__":
    api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    if not api_key:
        print("[Note] GEMINI_API_KEY environment variable is not set. Running boundary verification with offline fallback mode.")
